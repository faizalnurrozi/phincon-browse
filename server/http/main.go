...
func main() {
	...
	ucContract := usecase.Contract{
		...
		SvcFileStorageClient: config.SvcFileStorageClient,
	...
	// AddInformation Prometheus Middleware
	//prometheus := fiberprometheus.New("svc-product")
	//prometheus.RegisterAt(boot.App, "/metrics")
	//boot.App.Use(prometheus.Middleware)
